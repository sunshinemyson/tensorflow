...
// Wraps the ovx rewriter in a transform so it can be used as part of the
...
--out_graph=\
/tmp/tensorflow_inception_v3_stripped_optimized_quantized_fused_ovx.pb \
--inputs='Mul' \
--outputs='softmax' \
--transforms='\
rewrite_quantized_stripped_model_for_ovx(
...
Status RewriteQuantizedStrippedModelForOvx(
  ...
  *output_graph_def = GraphTransferUtils::BuildFusedGraphDef(
      OvxOpsDefinitions::getInstance(), "remote_fused_graph_execute_node",
      inputs, outputs, input_graph_def, &gt);
  return Status::OK();
}

REGISTER_GRAPH_TRANSFORM("rewrite_quantized_stripped_model_for_ovx",
                         RewriteQuantizedStrippedModelForOvx);
