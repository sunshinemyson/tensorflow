...
==============================================================================*/
// Tests in this file are designed to evaluate ovx DSP operations.
...
class QuantizedMatMulOpForOvxTest : public OpsTestBase {
 protected:
  void SetUp() final {
#ifdef USE_OVX_LIBS
    profile_utils::CpuUtils::EnableClockCycleProfiling(true);
    LOG(INFO) << "Ovx libs are linked (wrapper version = "
...
#else
    LOG(WARNING) << "Ovx libs are not linked.";
#endif
  }
};

// Shows some statistics of ovx dsp using ovx specific APIs
#ifdef USE_OVX_LIBS
TEST_F(QuantizedMatMulOpForOvxTest, EvaluateSharedLibOverhead) {
  ...
  const uint64 overhead_ovx_rpc_start =
      profile_utils::CpuUtils::GetCurrentClockCycle();
  const int ovx_binary_version = soc_interface_GetSocControllerVersion();
  const uint64 overhead_ovx_rpc_end =
      profile_utils::CpuUtils::GetCurrentClockCycle();
  const uint64 overhead_ovx_rpc_diff =
      (overhead_ovx_rpc_end - overhead_ovx_rpc_start);
  LOG(INFO) << "Shared lib (ver = " << wrapper_version << ") overhead is "
            ...
            << std::chrono::duration_cast<std::chrono::microseconds>(
                   ...
                   .count()
            << " usec";
  LOG(INFO) << "ovx rpc (ver = " << ovx_binary_version
            << ") overhead is " << overhead_ovx_rpc_diff
            << " cycles, time = "
            << std::chrono::duration_cast<std::chrono::microseconds>(
                   profile_utils::CpuUtils::ConvertClockCycleToTime(
                       overhead_ovx_rpc_diff))
                   .count()
            << " usec";
}
#endif
...
// This test is a sample to execute matmul on ovx.
TEST_F(QuantizedMatMulOpForOvxTest, Small_NoParams) {
