...
#include "tensorflow/core/kernels/ovx/graph_transfer_utils.h"
...
constexpr const char* const FUSED_MODEL_FILENAME =
    "/data/local/tmp/"
    "tensorflow_inception_v3_stripped_optimized_quantized_fused_ovx.pb";
...
static void CheckOvxControllerVersion() {
  OvxControlWrapper ovx_control_wrapper;
  const int version = ovx_control_wrapper.GetVersion();
  ASSERT_GE(version, 1);
  LOG(INFO) << "Ovx controller version is " << version;
}
...
static void RunInferenceByOvxControlWrapper(
  ...
  OvxControlWrapper ovx_control_wrapper;
  // 1. Initialize ovx
  ovx_control_wrapper.Init(execute_info);

  // 2. Setup graph in ovx
  ovx_control_wrapper.SetupGraph();

  // 3. Fill input node's output
  ovx_control_wrapper.FillInputNode("Mul", img_tensor);
  ...
  // 6. Teardown graph in ovx
  ovx_control_wrapper.TeardownGraph();

  // 7. Finalize ovx
  ovx_control_wrapper.Finalize();
}
...
// CAVEAT: This test only runs when you specify ovx library using
// makefile.
// CAVEAT: This test is disabled by default because ovx can keep only
...
// TODO(satok): Figure out how to release memory on ovx without process
// termination.
#ifdef USE_OVX_LIBS
TEST(GraphTransferer,
     DISABLED_RunInceptionV3OnOvxExampleWithOvxWrapper) {
  LOG(INFO) << "Run inception v3 on ovx with ovx controller";
  CheckOvxControllerVersion();
  ...
  std::vector<float> img_floats;
  LoadImage(&img_floats);
  RunInferenceByOvxControlWrapper(gt, img_floats);
}

TEST(GraphTransferer, RunInceptionV3OnOvxExampleWithTfRuntime) {
  LOG(INFO) << "Fuse and run inception v3 on ovx with tf runtime";
  CheckOvxControllerVersion();
...
TEST(GraphTransferer, DISABLED_RunInceptionV3OnOvxExampleWithFusedGraph) {
