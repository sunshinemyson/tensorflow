...
namespace ovx_remote_fused_graph_executor_build {

Status BuildRemoteFusedGraphExecutor(
    std::unique_ptr<IRemoteFusedGraphExecutor>* executor) {
  executor->reset(new OvxControlWrapper());
  return Status::OK();
}

static RemoteFusedGraphExecuteUtils::ExecutorBuildRegistrar
    k_ovx_remote_fused_graph_executor_build(
        "build_ovx_remote_fused_graph_executor",
