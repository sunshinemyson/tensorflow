...
#define OVX_CONTROLLER_VERSION 91
...
void ovx_controller_InitInputNodeDataToInceptionDummyData(int version) {
  if (version == 1) {
    ...
    ovx_controller_CopyByteNodeData(
...
bool ovx_controller_ExecuteGraphWithBuffer(
...
void ovx_controller_PrintGraph(uint32_t id) {
  int retval = ovx_nn_snpprint(id, s_print_buf, PRINT_BUFSIZE);
...
int ovx_controller_GetOvxBinaryVersion() {
  int retval = 0;
  ovx_nn_version(&retval);
  return retval;
}

bool ovx_controller_AllocateNodeDataBuffers(
...
bool ovx_controller_CopyByteNodeData(
...
int ovx_controller_InitOvxWithMaxAttributes(
    int enable_dcvs, int bus_usage, int version) {
  TFMLOGI("Init ovx with max attributes (Controller version = %d)",
          OVX_CONTROLLER_VERSION);
  const int MCPS = 1000;
  ...
  ovx_controller_AllocateNodeDataBuffers(
      INPUT_NODE_DATA_BUFFER_SIZE, OUTPUT_NODE_DATA_BUFFER_SIZE);

  if (s_dbg_use_inception_dummy_data) {
    ovx_controller_InitInputNodeDataToInceptionDummyData(version);
  }
  s_target_graph_id = 0;
...
int ovx_controller_AppendConstNode(
...
int ovx_controller_AppendNode(
  ...
  const int retval = ovx_nn_append_node(
