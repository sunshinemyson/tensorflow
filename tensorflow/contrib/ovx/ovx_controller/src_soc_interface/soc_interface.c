...
static ovx_nn_input* s_node_inputs_array;
static int s_node_inputs_array_index;
static int s_node_inputs_array_max_count;

static ovx_nn_output* s_node_outputs_array;
...
bool soc_interface_Init() {
  TFMLOGD("Init");
  ovx_controller_InitOvxWithMaxAttributes(
      0, 100, INCEPTION_VERSION /* version */);
  ovx_controller_GrowMemorySize();
  return true;
}
...
bool soc_interface_ExecuteGraph() {
  ...
  if (ovx_controller_IsDbgUseInceptionDummyDataEnabled()) {
    ovx_controller_InitInputNodeDataToInceptionDummyData(
  ...
  ovx_controller_ExecuteGraphWithBuffer(graph_id, true);
  return true;
}
...
bool soc_interface_TeardownGraph() {
  ...
  ovx_controller_Teardown(graph_id);
  return true;
}
...
bool soc_interface_FillInputNodeFloat(
  ...
  struct NodeDataFloat* node_data_float =
      ovx_controller_GetInputNodeDataFloatBuffer();
...
bool soc_interface_ReadOutputNodeFloat(
  ...
  float* output_node_data_float =
      ovx_controller_GetOutputNodeDataFloatBuffer(node_name, &array_size);
  if (array_size < 0) {
...
bool soc_interface_SetupGraphDummy(int version) {
  ...
  const uint32_t graph_id = ovx_controller_SetupGraph(version);
  if (graph_id == 0) {
    TFMLOGE("Failed to setup graph");
    return false;
  }
  ovx_controller_SetTargetGraphId(graph_id);
  return true;
}
...
bool soc_interface_AllocateNodeInputAndNodeOutputArray(
  ...
  s_node_inputs_array = malloc(total_input_count * sizeof(ovx_nn_input));
  s_node_outputs_array = malloc(total_output_count * sizeof(ovx_nn_output));
...
bool soc_interface_AppendConstNode(
  ...
  const uint32_t graph_id = ovx_controller_GetTargetGraphId();
  const int retval = ovx_controller_AppendConstNode(
...
bool soc_interface_AppendNode(
  ...
  const uint32_t graph_id = ovx_controller_GetTargetGraphId();
  const int retval = ovx_controller_AppendNode(
...
void soc_interface_SetDebugFlag(uint64_t flag) {
  ...
  if ((flag & FLAG_ENABLE_INCEPTION_DUMMY_BINARY_INPUT) != 0) {
    ...
    ovx_controller_EnableDbgUseInceptionDummyData(true);
  }
}
